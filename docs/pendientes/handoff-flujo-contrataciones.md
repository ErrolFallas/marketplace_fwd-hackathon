...
- ÚNICO PENDIENTE (decisión de diseño): la **barra slim del egresado** en `EntregablesClient` (líneas ~167-213) muestra badge de periodo (duplica `ContratoCardEgresado`) + fecha_inicio/fecha_fin (únicas) + repo URL (duplica el editor de la col izquierda). Opciones: quitarla entera (pierde las fechas visibles) / quitar solo lo duplicado y conservar fechas / dejarla. Sin decidir.
