...
- SIGUIENTE (falta el drill-down + rutas):
  1. **Rutas + drill-down:** lista compacta de entregables (título + badge de estado + "Ver propuestas") que reemplaza el render inline de `EntregablesTareas` en ambas zonas de trabajo; ruta de detalle `(rol)/contrataciones/[idProyecto]/entregables/[idEntregable]` que renderiza `TareaCard` de UN entregable (reusar TareaCard tal cual). Query nueva `getTareaDetalle(idTarea)` (tarea + propuestas + idProyecto + estado_periodo). Mover egresado a `egresado/contrataciones/[id]` (nuevo) + redirect de `projects/[id]/entregables` + reescribir ~5 links (MisContratacionesList, applications `hiredBannerCta`, puente en `MensajeriaWorkspace`, puente en `ProjectDetailClient`) + actualizar los `revalidatePath('/egresado/projects/${id}/entregables')` de las actions al nuevo path.
  2. **Remate:** empresario ve el repo global en `contrataciones/[id]`; quitar la barra slim duplicada del egresado. Skill design para la lista compacta.
