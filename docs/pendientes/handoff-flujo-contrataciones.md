...
- SIGUIENTE (backend listo, falta rutas + UI):
  1. **Rutas:** mover egresado a `egresado/contrataciones/[id]` (nuevo) + redirect de `projects/[id]/entregables` + reescribir ~5 links (MisContratacionesList, applications `hiredBannerCta`, puente en `MensajeriaWorkspace`, puente en `ProjectDetailClient`). Ruta de detalle `(rol)/contrataciones/[idProyecto]/entregables/[idEntregable]` para ambos roles.
  2. **UI:** lista compacta de entregables (título + badge Abierta/Requiere cambios/Cerrada/Finalizada + "Ver propuestas") + pantalla de detalle (encabezado + propuestas + form/vista multi-evidencia). `TareaCard` hoy sube con `file` único → reescribir el form a descripción(req)+link+dropzone multi-archivo, y la vista a "tira de evidencia" (link=primary/`ExternalLink`, PDF=secondary/`FileText`, imagen=accent/miniatura con `getSignedUrlAdjunto`). + empresario ve el repo global + quitar la barra slim duplicada del egresado. Skill design.
