...
- **RF-53 réplica — VISIBLE HECHO** `c284610`: la réplica del egresado (`evaluaciones.respuesta_evaluado`) dejó de ser callejón sin salida. Se sumó a `CalificacionRecibida` + `getMisCalificacionesRecibidas` (perfil propio) + `fetchCalificacionesByEstudiante` (vistas de la empresa sobre el candidato) + render en `ProfileView` (aparece en perfil egresado + `ranking/[id]` + `matches` + `portafolio-egresado`). La réplica **ya funcionaba en cancelado Y finalizado** (efecto de la tanda calificar-en-cancelado; el cuadro vive dentro del branch `finalizado||cancelado` y `addRespuestaEvaluacion` no gatea estado). Cero migración. 735 tests, build OK.
