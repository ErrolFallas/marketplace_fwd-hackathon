...
interface ContratacionFinalizadaData {
  nombre: string
...
/**
 * Correo al egresado cuando el empresario finaliza la contratación (RF-47/RF-49):
 * el proyecto quedó finalizado y ya puede calificar a la empresa. es-only, mismo
 * layout de marca FWD.
 */
export function contratacionFinalizadaHtml({
  nombre,
  tituloProyecto,
  urlContratacion,
}: ContratacionFinalizadaData): string {
  const nombreSafe = escapeHtml(nombre)
  const tituloSafe = escapeHtml(tituloProyecto)
  const urlSafe = escapeHtml(urlContratacion)
...
<head>
  ...
  <title>Proyecto finalizado — FWD Talent</title>
</head>
          ...
          <tr>
            <td style="background-color:#662D91;padding:32px 40px;">
              ...
              <h1 style="margin:8px 0 0;color:#ffffff;font-size:28px;font-weight:800;line-height:1.2;">
                Proyecto finalizado<span style="color:#FFCB05;">.</span>
              </h1>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:32px 40px;">
              ...
              <p style="margin:0 0 32px;color:#374151;font-size:15px;line-height:1.6;">
                El proyecto <strong>${tituloSafe}</strong> se finalizó. Gracias por
                tu trabajo. Ya podés calificar a la empresa y dejar tu reseña.
              </p>
...
export function contratacionFinalizadaSubject(tituloProyecto: string): string {
  return `El proyecto "${tituloProyecto}" se finalizó`
}
