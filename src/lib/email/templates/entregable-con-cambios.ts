...
interface EntregableConCambiosData {
  nombre: string
...
/**
 * Correo al egresado cuando el empresario solicita cambios sobre su entregable
 * (RF-44 / RF-46). El `comentario` con los cambios pedidos es el dato útil. No
 * es un "rechazo": el entregable vuelve a `con_cambios` para corregirse. es-only.
 */
export function entregableConCambiosHtml({
  nombre,
  tituloProyecto,
  urlEntregables,
  comentario,
}: EntregableConCambiosData): string {
  const nombreSafe = escapeHtml(nombre)
  const tituloSafe = escapeHtml(tituloProyecto)
  const urlSafe = escapeHtml(urlEntregables)
  const comentarioSafe = escapeHtml(comentario)
...
<head>
  ...
  <title>Cambios solicitados — FWD Talent</title>
</head>
          ...
          <tr>
            <td style="background-color:#662D91;padding:32px 40px;">
              ...
              <h1 style="margin:8px 0 0;color:#ffffff;font-size:28px;font-weight:800;line-height:1.2;">
                Cambios solicitados<span style="color:#FFCB05;">.</span>
              </h1>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:32px 40px;">
              ...
              <p style="margin:0 0 16px;color:#374151;font-size:15px;line-height:1.6;">
                El empresario revisó tu entregable del proyecto
                <strong>${tituloSafe}</strong> y solicitó algunos cambios:
              </p>
              <p style="margin:0 0 24px;padding:16px;background-color:#f4f4f5;border-radius:12px;color:#374151;font-size:14px;line-height:1.6;">${comentarioSafe}</p>
...
export function entregableConCambiosSubject(tituloProyecto: string): string {
  return `Pidieron cambios en tu entregable de "${tituloProyecto}"`
}
